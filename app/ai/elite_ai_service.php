        ...
        try {
            db_query(
                "CREATE TABLE IF NOT EXISTS elite_ai_audit_logs (
                    ...
                    planner_provider VARCHAR(40) NOT NULL DEFAULT '',
                    planner_intent VARCHAR(60) NOT NULL DEFAULT '',
                    execution_intent VARCHAR(60) NOT NULL DEFAULT '',
                    execution_status VARCHAR(40) NOT NULL DEFAULT '',
                    mismatch_reason VARCHAR(120) NOT NULL DEFAULT '',
            ...
            $auditColumns = [
                'planner_provider' => "ALTER TABLE elite_ai_audit_logs ADD COLUMN planner_provider VARCHAR(40) NOT NULL DEFAULT '' AFTER page_context_json",
                'planner_intent' => "ALTER TABLE elite_ai_audit_logs ADD COLUMN planner_intent VARCHAR(60) NOT NULL DEFAULT '' AFTER planner_provider",
                'execution_intent' => "ALTER TABLE elite_ai_audit_logs ADD COLUMN execution_intent VARCHAR(60) NOT NULL DEFAULT '' AFTER planner_intent",
                'execution_status' => "ALTER TABLE elite_ai_audit_logs ADD COLUMN execution_status VARCHAR(40) NOT NULL DEFAULT '' AFTER execution_intent",
                'mismatch_reason' => "ALTER TABLE elite_ai_audit_logs ADD COLUMN mismatch_reason VARCHAR(120) NOT NULL DEFAULT '' AFTER execution_status",
            ];
            ...
            foreach ($auditColumns as $column => $sql) {
                try {
                    $exists = (int) db_value(
                        ...
                        ['table' => 'elite_ai_audit_logs', 'column' => $column]
                    );
                    if ($exists === 0) {
                        db_query($sql);
                    }
                } catch (Throwable $e) {
                    esm_log('elite_ai', 'Could not ensure Elite AI audit column.', [
                        'column' => $column,
                        'error' => $e->getMessage(),
                    ]);
                }
...
if (!function_exists('elite_ai_prompt_requests_send_pending_drafts')) {
    function elite_ai_prompt_requests_send_pending_drafts(string $prompt): bool
    {
        $normalized = strtolower(trim($prompt));
        if ($normalized === '') {
            return false;
        ...
        return (bool) preg_match('/\b(?:send|dispatch|deliver)\s+(?:them|those|these|all|all\s+of\s+them|the\s+drafts?|the\s+messages?)\b|\b(?:send|dispatch|deliver)\s+(?:all|the|these|those)\s+(?:sms|texts?|messages?|drafts?)\b/i', $normalized);
    }
}

if (!function_exists('elite_ai_send_sms_draft_item')) {
    function elite_ai_send_sms_draft_item(array $user, array $draft): array
    {
        $actionId = (int) ($draft['action_id'] ?? 0);
        $leadId = (int) ($draft['lead_id'] ?? 0);
        $payload = (array) ($draft['draft'] ?? []);
        $message = trim((string) ($payload['reply'] ?? $payload['message'] ?? $payload['text'] ?? $payload['body'] ?? $draft['draft_preview'] ?? ''));
        if ($actionId <= 0 || $leadId <= 0 || $message === '') {
            return ['ok' => false, 'lead_id' => $leadId, 'action_id' => $actionId, 'message' => 'Draft is missing lead, queue id, or SMS body.'];
        }
        ...
        if (trim((string) ($lead['phone'] ?? '')) === '') {
            return ['ok' => false, 'lead_id' => $leadId, 'action_id' => $actionId, 'message' => 'Lead has no phone number.'];
        }
        if (trim((string) ($lead['sms_opt_status'] ?? 'unknown')) === 'opted_out') {
            return ['ok' => false, 'lead_id' => $leadId, 'action_id' => $actionId, 'message' => 'Lead opted out of SMS.'];
        }
        if (!function_exists('elite_twilio_send_sms')) {
            return ['ok' => false, 'lead_id' => $leadId, 'action_id' => $actionId, 'message' => 'SMS sender is unavailable.'];
        }

        $sendResult = elite_twilio_send_sms((string) ($lead['phone'] ?? ''), $message, [
            'lead_id' => $leadId,
            'lead' => $lead,
            'send_pushover_fallback' => true,
            'fallback_summary' => 'Twilio could not send the Elite AI approved SMS draft. Open lead actions to retry manually.',
            'original_body' => $message,
        ]);
        if (empty($sendResult['ok'])) {
            return ['ok' => false, 'lead_id' => $leadId, 'action_id' => $actionId, 'message' => (string) ($sendResult['message'] ?? 'SMS failed.')];
        }
        ...
        if (function_exists('lead_comm_insert_activity')) {
            lead_comm_insert_activity($leadId, 'sms_outbound', 'Elite AI sent approved SMS draft.', [
        ...
        return [
            ...
            'message' => 'SMS sent.',
            'twilio_sid' => (string) ($sendResult['twilio_sid'] ?? ''),
        ];
    }
}
...
if (!function_exists('elite_ai_send_pending_drafts_payload')) {
    function elite_ai_send_pending_drafts_payload(array $user, array $context, string $prompt, string $surface): ?array
    {
        ...
        foreach (array_reverse((array) ($context['assistant_thread'] ?? [])) as $item) {
            ...
            $role = strtolower(trim((string) ($item['role'] ?? 'assistant')));
            $text = trim((string) ($item['text'] ?? ''));
            if ($role === 'assistant' && $text !== '' && stripos($text, 'prepared') !== false && stripos($text, 'draft') !== false) {
                $threadText = strtolower($text);
                break;
            }
        }
        if ($threadText !== '') {
            $pending = array_values(array_filter($pending, static function (array $draft) use ($threadText): bool {
                $name = strtolower(trim((string) ($draft['lead_name'] ?? '')));
                return $name !== '' && str_contains($threadText, $name);
            }));
        ...
        if (!$pending) {
            return [
                'answer' => 'I found the previous prepared-draft message, but I do not see matching pending drafts anymore. They may already be used, cancelled, or sent.',
                'cards' => [],
                'actions' => [],
                'tools_used' => ['pending_draft_context'],
            ];
        ...
        foreach ($pending as $draft) {
            ...
            if (!empty($result['ok'])) {
                ...
                $failed[] = trim((string) ($draft['lead_name'] ?? 'Lead #' . (int) ($draft['lead_id'] ?? 0))) . ' - ' . trim((string) ($result['message'] ?? 'failed'));
            }
        }

        $items = [];
        foreach ($sent as $row) {
            $items[] = trim((string) ($row['lead_name'] ?? 'Lead #' . (int) ($row['lead_id'] ?? 0))) . ' - sent.';
        }
        ...
        return [
            ...
            'actions' => [],
            'tools_used' => ['pending_draft_context', 'lead.sms_send', 'lead_messages'],
...
if (!function_exists('elite_ai_request_has_explicit_send_permission')) {
    function elite_ai_request_has_explicit_send_permission(array $request, string $channel = ''): bool
    {
        $channel = strtolower(trim($channel));
        if ($channel === 'sms') {
            if (filter_var($request['sms_send_approved'] ?? $request['send_sms_approved'] ?? false, FILTER_VALIDATE_BOOLEAN)) {
                return true;
            }
            if (filter_var($request['email_send_approved'] ?? $request['send_email_approved'] ?? false, FILTER_VALIDATE_BOOLEAN)) {
                return false;
            }
        }
        if ($channel === 'email') {
        ...
        $approvedChannel = strtolower(trim((string) ($request['approved_channel'] ?? '')));
        ...
        if ($channel !== '') {
            $mentionsSms = (bool) preg_match('/\\b(?:sms|text|text message)\\b/i', $normalizedInstruction);
            $mentionsEmail = (bool) preg_match('/\\b(?:email|e mail)\\b/i', $normalizedInstruction);
            if (($channel === 'sms' && $mentionsEmail && !$mentionsSms)
                || ($channel === 'email' && $mentionsSms && !$mentionsEmail)) {
                return false;
            }
        }
...
if (!function_exists('elite_ai_request_has_explicit_stage_approval')) {
    function elite_ai_request_has_explicit_stage_approval(array $request): bool
    {
        ...
        return elite_ai_prompt_requests_stage_move($instruction);
    }
}
...
if (!function_exists('elite_ai_request_has_explicit_schedule_approval')) {
    function elite_ai_request_has_explicit_schedule_approval(array $request): bool
    {
        ...
        $instruction = strtolower(trim((string) ($request['instruction'] ?? '')));
        return $instruction !== '' && elite_ai_prompt_requests_consult_schedule($instruction);
    }
}
...
if (!function_exists('elite_ai_execution_policy_tag')) {
    ...
    {
        if (filter_var($request['sms_send_approved'] ?? $request['send_sms_approved'] ?? false, FILTER_VALIDATE_BOOLEAN)) {
            return 'sms-send-approved';
        }
        if (filter_var($request['email_send_approved'] ?? $request['send_email_approved'] ?? false, FILTER_VALIDATE_BOOLEAN)) {
            return 'email-send-approved';
        }
...
if (!function_exists('elite_ai_stage_aliases')) {
    function elite_ai_stage_aliases(): array
    {
        return [
            'new_lead' => ['new lead'],
            'attempted_contact' => ['first touch attempted', 'first touch attempt', 'attempted contact'],
            'contacted' => ['first touch sent', 'first touch', 'contacted'],
            'in_contact' => ['active follow up', 'scheduling', 'schedule', 'schudele', 'schudle', 'in communication', 'in contact'],
            'consultation_booked' => ['consultation booked', 'consult booked', 'booked'],
            'no_show_reschedule' => ['no show reschedule', 'no show', 'reschedule'],
            'consult_completed' => ['consultation completed', 'consult completed', 'completed consult'],
            'treatment_accepted' => ['treatment accepted', 'sale closed', 'accepted'],
            'treatment_completed' => ['treatment completed', 'completed treatment', 'completed paid', 'paid completed', 'case completed'],
            'no_answer' => ['no answer nurture', 'no answer', 'nurture', 'follow later'],
            'opted_out' => ['opted out', 'unsubscribe', 'stop'],
            'lost_lead' => ['lost archived', 'lead lost', 'archived', 'archive', 'lost'],
        ];
    }
}

if (!function_exists('elite_ai_normalize_stage_text')) {
    function elite_ai_normalize_stage_text(string $text): string
    {
...
if (!function_exists('elite_ai_stage_from_text')) {
    ...
    {
        $normalized = elite_ai_normalize_stage_text($text);
        if ($normalized === '') {
            return '';
        }
        ...
        foreach ($aliases as $alias => $stage) {
            if ((bool) preg_match('/(?:^|\s)' . preg_quote($alias, '/') . '(?:$|\s)/i', $normalized)) {
                return $stage;
            }
        }
...
if (!function_exists('elite_ai_plan_target_stage')) {
    function elite_ai_plan_target_stage(array $plan, string $prompt = ''): string
    {
        $candidate = elite_ai_stage_from_text((string) ($plan['target_stage'] ?? ''));
        if ($candidate === '' && $prompt !== '') {
            $candidate = elite_ai_requested_stage_key($prompt);
        }
        $allowedStages = function_exists('lead_stage_labels') ? lead_stage_labels() : [];
        return $candidate !== '' && isset($allowedStages[$candidate]) ? $candidate : '';
    }
}
...
if (!function_exists('elite_ai_prompt_requests_stage_count')) {
    function elite_ai_prompt_requests_stage_count(string $prompt): ?string
    {
        ...
        $stage = elite_ai_stage_from_text($normalized);
...
if (!function_exists('elite_ai_build_draft_preview_actions')) {
    function elite_ai_build_draft_preview_actions(int $leadId, int $actionId, string $draftType = ''): array
    {
        $channel = $draftType === 'draft_email' ? 'email' : ($draftType === 'draft_sms' ? 'sms' : '');
        $base = [
            ['type' => 'send_draft', 'label' => 'Send Draft', 'lead_id' => $leadId, 'action_id' => $actionId, 'channel' => $channel],
...
if (!function_exists('elite_ai_pending_action_row')) {
    function elite_ai_pending_action_row(array $row): array
    {
        ...
        return [
            ...
            'actions' => elite_ai_build_draft_preview_actions($leadId, (int) ($row['id'] ?? 0), $actionType),
        ];
    }
}
...
if (!function_exists('elite_ai_requested_stage_key')) {
    ...
    {
        if (preg_match('/\b(?:to|as|into)\s+(.+?)\s*$/i', trim($text), $matches)) {
            $target = elite_ai_stage_from_text((string) ($matches[1] ?? ''));
            if ($target !== '') {
                return $target;
            }
        }
        return elite_ai_stage_from_text($text);
    }
}
...
if (!function_exists('elite_ai_prompt_is_historical_stage_statement')) {
    ...
    {
        $normalized = strtolower(trim($prompt));
        if ($normalized === '' || elite_ai_requested_stage_key($normalized) === '') {
            return false;
        }

        $pastVerb = '(?:moved|sent|put|set|changed|marked|advanced|shifted)';
        $temporalPast = (bool) preg_match('/\b(?:already|previously|earlier|yesterday|last\s+night|this\s+morning|this\s+afternoon|just)\b.{0,80}\b' . $pastVerb . '\b|\b' . $pastVerb . '\b.{0,80}\b(?:already|previously|earlier|yesterday|last\s+night|this\s+morning|this\s+afternoon)\b/i', $normalized);
        $passivePast = (bool) preg_match('/\b(?:was|were|has\s+been|have\s+been|had\s+been)\s+' . $pastVerb . '\b/i', $normalized);
        $operatorPast = (bool) preg_match('/\b(?:i|we)\s+' . $pastVerb . '\b/i', $normalized);
        $subjectPast = (bool) preg_match('/^(?!.*\b(?:should|must|needs?\s+to)\s+be\b)(?!.*\b(?:please\s+)?(?:have|get)\b).+\s+' . $pastVerb . '\s+(?:to|as|into)\b/i', $normalized);
        $currentState = (bool) preg_match('/\b(?:is|are)\s+now\b/i', $normalized);

        return $temporalPast || $passivePast || $operatorPast || $subjectPast || $currentState;
    }
}
...
if (!function_exists('elite_ai_prompt_requests_stage_move')) {
    function elite_ai_prompt_requests_stage_move(string $prompt): bool
    {
        ...
        $imperative = (bool) preg_match('/\b(?:move|send|put|set|change|mark|advance|shift)\b.+\b(?:to|as|into)\b/i', $normalized);
        $passiveDirective = (bool) preg_match('/\b(?:should|must|needs?\s+to)\s+be\s+(?:moved|sent|put|set|changed|marked|advanced|shifted)\b.+\b(?:to|as|into)\b/i', $normalized);
        $causativeDirective = (bool) preg_match('/\b(?:please\s+)?(?:have|get)\b.+\b(?:moved|sent|put|set|changed|marked|advanced|shifted)\s+(?:to|as|into)\b/i', $normalized);
        return ($imperative || $passiveDirective || $causativeDirective) && elite_ai_requested_stage_key($normalized) !== '';
    }
}
...
if (!function_exists('elite_ai_extract_stage_move_lead_query')) {
    function elite_ai_extract_stage_move_lead_query(string $prompt): string
    {
        ...
        $patterns = [
            '/^(.+?)\s+(?:should|must|needs?\s+to)\s+be\s+(?:moved|sent|put|set|changed|marked|advanced|shifted)\s+(?:to|as|into)\s+.+$/i',
            '/\b(?:move|send|put|set|change|mark|advance|shift)\s+(.+?)\s+(?:to|as|into)\s+.+$/i',
            '/^(.+?)\s+(?:to|as|into)\s+.+$/i',
        ];
    ...
    function elite_ai_prepare_action_draft(array $user, array $request, string $surface): array
    {
        ...
        if (in_array($actionType, ['send_draft', 'use_draft', 'edit_draft', 'cancel_draft'], true)) {
            ...
            if ($actionType === 'cancel_draft') {
                if (strcasecmp((string) ($context['page'] ?? ''), 'elite-ai-lab') === 0 && empty($request['allow_live_cancel'])) {
                    return [
                        ...
                        'draft_actions' => elite_ai_build_draft_preview_actions($leadId, $actionId, $draftType),
            ...
            if ($actionType === 'send_draft') {
                ...
                $sendChannel = $draftType === 'draft_email' ? 'email' : 'sms';
                if (!elite_ai_request_has_explicit_send_permission($request, $sendChannel)) {
                    return [
                        'ok' => true,
                        'surface' => $surface,
                        'action' => 'send_draft',
                ...
                return [
                    ...
                    'draft_actions' => !empty($sendResult['ok']) ? [] : elite_ai_build_draft_preview_actions($leadId, $actionId, $draftType),
                ];
            }
        ...
        foreach ($existingDrafts as $existingDraft) {
            ...
            return [
                ...
                'draft_badge' => 'Draft only - not sent',
                'draft_actions' => elite_ai_build_draft_preview_actions($leadId, $existingActionId, $actionType),
                'status' => 'pending_review',
        ...
        if ($actionType === 'draft_sms') {
            ...
            return [
                ...
                'draft_badge' => 'Draft only - not sent',
                'draft_actions' => elite_ai_build_draft_preview_actions($leadId, $actionId, 'draft_sms'),
                'status' => 'pending_review',
        ...
        if ($actionId <= 0) {
            ...
            return [
                ...
                'draft_preview' => $draftPreview,
                'draft_badge' => 'Draft only - not sent',
                'draft_actions' => elite_ai_build_draft_preview_actions($leadId, $actionId, 'draft_email'),
                'status' => 'pending_review',
...
if (!function_exists('elite_ai_handle_action_request')) {
    function elite_ai_handle_action_request(array $user, array $request): array
    {
        ...
        if ($assistantAction === 'move_stage') {
            ...
            $executionObservation = elite_ai_execution_observation(['provider' => 'action', 'intent' => 'move_stage'], 'move_stage', $result, (int) ($result['lead_id'] ?? 0) ?: null);
        ...
        if ($assistantAction === 'add_note') {
            ...
            $result = elite_ai_plain_text_payload($result);
            $executionObservation = elite_ai_execution_observation(['provider' => 'action', 'intent' => 'add_note'], 'add_note', $result, (int) ($result['lead_id'] ?? 0) ?: null);
            elite_ai_log_interaction(
                $user,
                $surface,
                (string) ($request['prompt'] ?? $request['instruction'] ?? ''),
        ...
        if ($assistantAction === 'schedule_consultation') {
            ...
            $result = elite_ai_plain_text_payload($result);
            $executionObservation = elite_ai_execution_observation(['provider' => 'action', 'intent' => 'schedule_consultation'], 'schedule_consultation', $result, (int) ($result['lead_id'] ?? 0) ?: null);
            elite_ai_log_interaction(
                $user,
                $surface,
                (string) ($request['prompt'] ?? $request['instruction'] ?? ''),
        ...
        if ($assistantAction === 'mark_reviewed') {
            ...
            $result = elite_ai_plain_text_payload($result);
            $executionObservation = elite_ai_execution_observation(['provider' => 'action', 'intent' => 'mark_reviewed'], 'mark_reviewed', $result, $leadId ?: null);
            elite_ai_log_interaction(
                $user,
                $surface,
        ...
        if (!empty($result['ok'])) {
            ...
            $result = elite_ai_plain_text_payload($result);
            $executionIntent = $assistantAction !== '' ? $assistantAction : (string) ($result['action'] ?? 'draft');
            $executionObservation = elite_ai_execution_observation(['provider' => 'action', 'intent' => $executionIntent], $executionIntent, $result, (int) ($result['lead_id'] ?? 0) ?: null);
            elite_ai_log_interaction(
                $user,
                $surface,
                (string) ($request['prompt'] ?? ''),
    ...
    function elite_ai_planner_schema(): array
    {
        return [
            ...
            'properties' => [
                ...
                'target_stage' => [
                    'type' => 'string',
                    'enum' => array_merge([''], array_keys(function_exists('lead_stage_labels') ? lead_stage_labels() : [])),
                ],
                'use_current_lead' => ['type' => 'boolean'],
                'needs_clarification' => ['type' => 'boolean'],
                'clarification_question' => ['type' => 'string'],
            ],
            'required' => ['intent', 'reason', 'lead_query', 'target_stage', 'use_current_lead', 'needs_clarification', 'clarification_question'],
...
if (!function_exists('elite_ai_planner_system_prompt')) {
    function elite_ai_planner_system_prompt(): string
    {
        return implode("\n", [
            ...
            'For move_stage, return the canonical CRM stage key in target_stage. For every other intent, return an empty target_stage.',
            'Valid target_stage keys: ' . implode(', ', array_keys(function_exists('lead_stage_labels') ? lead_stage_labels() : [])) . '.',
...
if (!function_exists('elite_ai_plan_request')) {
    ...
    function elite_ai_plan_request(string $prompt, string $quickAction, array $context): array
    {
        ...
        return [
            ...
            'target_stage' => elite_ai_plan_target_stage((array) $result['data']),
            ...
            if (count($matches) === 1) {
                if ((int) ($matches[0]['_elite_ai_match_distance'] ?? 0) > 0) {
                    ...
                    $targetStage = (string) ($plan['intent'] ?? '') === 'move_stage'
                        ? elite_ai_plan_target_stage($plan, $prompt)
                        : '';
...
if (!function_exists('elite_ai_execution_observation')) {
    function elite_ai_execution_observation(array $plan, string $executionIntent, array $payload, ?int $leadId): array
    {
        $plannerIntent = trim((string) ($plan['intent'] ?? ''));
        $provider = trim((string) ($plan['provider'] ?? ''));
        $status = 'completed';
        $reason = '';
        ...
        } elseif ($plannerIntent !== '' && $executionIntent !== '' && $plannerIntent !== $executionIntent) {
            $status = 'routed_override';
            $reason = 'planner_execution_intent_changed';
        } elseif (!empty($payload['requires_approval'])) {
            $status = 'approval_required';
            $reason = $executionIntent . '_approval_required';
        } elseif (array_key_exists('ok', $payload) && empty($payload['ok'])) {
            $status = 'failed';
            $reason = $executionIntent . '_execution_failed';
        } elseif ($executionIntent === 'move_stage' && in_array('stage_move_parse', (array) ($payload['tools_used'] ?? []), true)) {
            $status = 'clarification';
            $reason = 'missing_target_stage';
        } elseif ($leadId === null && in_array('lead_lookup', (array) ($payload['tools_used'] ?? []), true)) {
            $status = 'clarification';
            $reason = 'lead_resolution_failed_or_ambiguous';
        } elseif (in_array($executionIntent, ['move_stage', 'schedule_consultation', 'send_draft', 'add_note', 'mark_reviewed'], true)) {
            $status = 'executed';
        }

        return [
            'planner_provider' => mb_substr($provider, 0, 40),
            'planner_intent' => mb_substr($plannerIntent, 0, 60),
            'execution_intent' => mb_substr($executionIntent, 0, 60),
            'execution_status' => mb_substr($status, 0, 40),
            'mismatch_reason' => mb_substr($reason, 0, 120),
        ];
    }
}

if (!function_exists('elite_ai_log_interaction')) {
    function elite_ai_log_interaction(array $user, string $surface, string $prompt, array $toolsUsed, string $responseSummary, ?int $leadId, array $context, array $execution = []): void
    {
        try {
            elite_ai_ensure_schema();
            db_insert(
                'INSERT INTO elite_ai_audit_logs
                    (user_id, surface, prompt, tools_used_json, response_summary, lead_id, page_context_json, planner_provider, planner_intent, execution_intent, execution_status, mismatch_reason, created_at)
                 VALUES
                    (:user_id, :surface, :prompt, :tools_used_json, :response_summary, :lead_id, :page_context_json, :planner_provider, :planner_intent, :execution_intent, :execution_status, :mismatch_reason, :created_at)',
                [
                    'user_id' => (int) ($user['id'] ?? 0),
                    ...
                    'planner_provider' => mb_substr(trim((string) ($execution['planner_provider'] ?? '')), 0, 40),
                    'planner_intent' => mb_substr(trim((string) ($execution['planner_intent'] ?? '')), 0, 60),
                    'execution_intent' => mb_substr(trim((string) ($execution['execution_intent'] ?? '')), 0, 60),
                    'execution_status' => mb_substr(trim((string) ($execution['execution_status'] ?? '')), 0, 40),
                    'mismatch_reason' => mb_substr(trim((string) ($execution['mismatch_reason'] ?? '')), 0, 120),
                    'created_at' => now(),
                ]
            );
...
function elite_ai_handle_request(array $user, array $request): array
{
        ...
        if ($quickAction === '' && $prompt !== '') {
            $sendPendingPayload = elite_ai_send_pending_drafts_payload($user, $context, $prompt, $surface);
            if ($sendPendingPayload !== null) {
                $sendPendingPayload = elite_ai_plain_text_payload($sendPendingPayload);
                $summary = trim((string) ($sendPendingPayload['answer'] ?? 'Pending draft send task completed.'));
                elite_ai_log_interaction(
                    $user,
                    $surface,
                    $prompt,
                    (array) ($sendPendingPayload['tools_used'] ?? []),
                    $summary,
                    null,
                    $context
                );
                ...
                return [
                    ...
                    'answer' => $summary,
                    'execution_policy' => elite_ai_execution_policy_tag($request),
                    'pending_drafts' => array_values((array) ($sendPendingPayload['pending_drafts'] ?? elite_ai_pending_drafts_for_user($user, 8))),
                    'cards' => array_values((array) ($sendPendingPayload['cards'] ?? [])),
                    'actions' => array_values((array) ($sendPendingPayload['actions'] ?? [])),
                    'tools_used' => array_values((array) ($sendPendingPayload['tools_used'] ?? [])),
                    'tool_capabilities' => elite_ai_tool_capabilities($surface),
                    'lead_id' => null,
                    'current_subject' => [],
                    'context' => $context,
        ...
        if ($quickAction === '' && elite_ai_prompt_requests_pending_draft_review($prompt)) {
            ...
            if ($draftPayload) {
                ...
                return [
                    ...
                    'action' => trim((string) ($draftPayload['action'] ?? '')),
                    'action_id' => (int) ($draftPayload['action_id'] ?? 0) ?: null,
                    'action_type' => trim((string) ($draftPayload['action_type'] ?? '')),
                    'channel' => trim((string) ($draftPayload['channel'] ?? '')),
                    'draft' => (array) ($draftPayload['draft'] ?? []),
                    'payload' => (array) ($draftPayload['payload'] ?? []),
                    'draft_preview' => trim((string) ($draftPayload['draft_preview'] ?? '')),
                    'draft_badge' => trim((string) ($draftPayload['draft_badge'] ?? '')),
                    'draft_actions' => array_values((array) ($draftPayload['draft_actions'] ?? [])),
                    'warning' => $draftPayload['warning'] ?? null,
        ...
        if ($quickAction === '' && $prompt !== '' && elite_ai_prompt_requests_stage_move($prompt)) {
            $targetStage = elite_ai_plan_target_stage(['target_stage' => elite_ai_requested_stage_key($prompt)], $prompt);
            $stagePlan = [
                'intent' => 'move_stage',
            ...
            if ($targetStage !== '' && !empty($resolved['lead']) && is_array($resolved['lead'])) {
                ...
                $moveResult = elite_ai_plain_text_payload($moveResult);
                $summary = trim((string) ($moveResult['answer'] ?? $moveResult['message'] ?? 'Stage action completed.'));
                $executionObservation = elite_ai_execution_observation(
                    $stagePlan,
                    'move_stage',
                    $moveResult + ['tools_used' => ['lead.lookup', 'lead.move_stage']],
                    $leadId
                );
                elite_ai_log_interaction($user, $surface, $prompt, ['lead.lookup', 'lead.move_stage'], $summary, $leadId, $context, $executionObservation);
        ...
        switch ($intent) {
            ...
            case 'move_stage':
                $targetStage = elite_ai_plan_target_stage($plan, $prompt);
                $resolved = elite_ai_resolve_lead_from_plan($plan, $prompt, $context);
                if ($targetStage === '') {
                    $payload = [
        ...
        $executionObservation = elite_ai_execution_observation($plan, $intent, $payload, $leadId);
        elite_ai_log_interaction($user, $surface, $prompt !== '' ? $prompt : $quickAction, (array) ($payload['tools_used'] ?? []), $summary, $leadId, $context, $executionObservation);
