...
function smile_design_shade_detail(string $shadeGoal, string $style = 'natural'): array
{
    ...
    $detail['prompt'] = $detail['label'] . ' (' . $detail['title'] . '): ' . $detail['description'] . ' Digital screen reference ' . $detail['hex'] . ' / RGB(' . $detail['rgb'] . '). ' . $relativeValue . ' ' . $detail['screen_contract'] . ' Default veneer material: IPS e.max-style lithium disilicate glass-ceramic. Render a glazed ceramic surface with clean high-value body shade, realistic specular gloss, enamel-like depth, smooth polished finish, and subtle incisal translucency only at the bottom edge. The shade selector controls brightness and warmth; the lithium disilicate material controls finish and optical depth. Keep the veneer body clean and uniform in this selected shade. For on-screen consultation previews, the result must still read as polished high-end porcelain rather than cleaned natural teeth.';
    return $detail;
}
