...
try {
    ...
    if ($consultationStatus === 'scheduled' && $consultationDate !== null) {
        ...
        // Patient-facing confirmation (date, time, address, directions). Only
        // when the appointment itself is new or moved - the internal doctor
        // SMS above fires on every scheduled save, but the patient must not be
        // re-texted because someone corrected a phone number.
        $previousConsultationDate = trim((string) ($leadRow['consultation_date'] ?? ''));
        if ($previousConsultationDate !== (string) $consultationDate && function_exists('consultation_booking_confirmation_send')) {
            try {
                $confirmationLead = db_one('SELECT * FROM leads WHERE id = :id LIMIT 1', ['id' => $leadId]);
                if ($confirmationLead) {
                    consultation_booking_confirmation_send($confirmationLead, ['source' => 'lead_update_details']);
                }
            } catch (Throwable $confirmationError) {
                esm_log('appointment_confirmation', 'Appointment saved, but patient confirmation failed.', [
                    'lead_id' => $leadId,
