...
<body class="min-h-screen bg-slate-50 text-slate-900 antialiased">
    ...
    <main class="px-4 py-6 sm:px-6 lg:pl-80 lg:pr-8 lg:py-8">
        ...
        <section class="mb-8 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <h2 class="mt-2 text-xl font-semibold text-slate-950">Scan this QR to test the iPad</h2>
            <p class="mt-3 text-sm leading-6 text-slate-600">This is a ready-to-use test kiosk so you can scan once and verify the iPad flow before creating the final one.</p>
            <div class="mt-5 flex flex-col gap-4 md:flex-row md:items-center">
                <img src="<?= e((string)$testKioskSetup['setup_qr_url']) ?>" alt="Test kiosk QR code" class="h-44 w-44 rounded-2xl border border-slate-200 bg-white p-2">
                <div class="min-w-0 flex-1 space-y-3">
                    ...
                    <div class="flex flex-col gap-2 sm:flex-row">
                        <input value="<?= e((string)$testKioskSetup['setup_url']) ?>" readonly class="min-h-12 w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-xs text-slate-700 outline-none">
                        <button type="button" class="copy-setup-link min-h-12 rounded-2xl border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-100" data-copy-value="<?= e((string)$testKioskSetup['setup_url']) ?>">Copy test link</button>
                    </div>
                    <p class="text-xs leading-6 text-slate-500">Use this only for testing. After it works, create the final kiosk above.</p>
                </div>
            </div>
        </section>
