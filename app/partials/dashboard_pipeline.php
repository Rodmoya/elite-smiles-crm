...
(function () {
    ...
    function defaultEmailBody(card) {
        ...
        return applyTemplateTokens([
            ...
            '',
            'Every smile is different. During a complimentary consultation, Dr. Meden reviews your teeth, bite, photos, and goals before explaining which options may fit.',
            '',
            'You can reply here whenever a question comes up. Rod will also text you so it is easy to continue the conversation.',
            '',
            'Warmly,',
...
function applyCommunicationViewportFit() {
    ...
    document.addEventListener('click', function (event) {
        ...
        const leadId = queueButton.dataset.openActionLead || '';
        const card = safeCardLookupById(leadId);
        if (!card) return;
        ...
        window.setTimeout(() => {
            openLeadModal(card, queueButton.dataset.openActionTab || 'communications');
        }, 250);
    });
