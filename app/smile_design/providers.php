    ...
    function smile_design_smile_geometry_prompt_lines(array $analysis, string $targetPhotoType): array
    {
        ...
        $lines = [
            ...
            $isFront
                ...
                : '',
            'Smile arc: the incisal edges from canine to canine must follow the upward curve of the lower lip (a consonant smile arc). The centrals are the longest, the laterals are visibly shorter so their edges step up above the centrals, and the canines drop slightly again before the arch turns toward the corners. A flat line of equal-length edges reads older and artificial and is a failure - the stepped difference between central and lateral must be obvious at normal viewing distance.',
...
final class GoogleGeminiSmileDesignImageProvider implements SmileDesignImageProvider
{
    public function createPreview(array $case, array $photos, array $options = []): array
    {
        ...
        // Only Elite Smiles 100 / Chromascop 110 are meant to chase the maximum
        // wow-factor brightness look. Every other shade should render true to its
        // own Chromascop value, so the blanket "push it whiter" directives below
        // are gated to this anchor instead of firing for every shade.
        $shadeIsMaxBrightAnchor = (int)($shadeDetail['value_step'] ?? 4) <= 0;
        ...
        $promptParts = [
            ...
            ($isVeneerSimulation ? 'Material failure rule: if the veneers look flat, opaque, chalky, or like solid white paint instead of genuine glass-ceramic porcelain with real light transmission and depth, the material rendering has failed. The surface must show natural light scattering and translucent depth at the incisal edge — like real e.max lithium disilicate glass-ceramic, not a flat matte or plastic finish — regardless of which shade is selected.' : ''),
            ($isVeneerSimulation ? 'Clinical crown length target: give each upper central incisor a full, age-appropriate clinical crown length of roughly 10.5 mm to 11 mm, in a 4:5 width-to-length ratio where the vertical height clearly dominates the horizontal width. When the patient\'s own teeth are worn short, lengthen them toward that target instead of preserving the short display - restoring worn length is part of the treatment being previewed, not an exaggeration of it.' : ''),
            ($isVeneerSimulation ? 'Length failure rule: crown height must clearly dominate crown width - slender and refined, never bulky. Uniform, square, equal-length "chiclet" blocks across the front teeth are a failure, and so are teeth left so short they read stubby. In this smiling photo the central incisal edges should rest just along the inner curve of the lower lip: never hidden behind the lower lip, and never crossing over it. An exaggerated, denture-like smile line remains a failure in the other direction.' : ''),
            ($isVeneerSimulation ? 'Veneer transformation strength: the after must look like new ceramic veneers were placed at the selected shade\'s exact target color, not like the original teeth were simply cleaned or lightly whitened.' : ''),
            ($isVeneerSimulation && $shadeIsMaxBrightAnchor ? 'For this shade, the visible treated teeth should be at least two obvious screen-value steps whiter than the before photo while still retaining porcelain dimension and highlights.' : ''),
            ($isVeneerSimulation ? 'Shade accuracy failure rule: if the new veneers do not match the requested shade\'s target color and value — whether they remain too close to the original tooth color, or overshoot brighter/whiter than the requested shade — the edit has failed. Match the requested Chromascop (or Elite Smiles) target exactly, neither under- nor over-shooting it.' : ''),
            ($isVeneerSimulation && $shadeIsMaxBrightAnchor ? 'Brightness failure rule: if the new veneers look almost the same brightness as the original teeth or a conservative prior generated set, the edit has failed. Increase the white porcelain value until the before/after difference is obvious at normal viewing distance, while keeping dimensional porcelain rather than flat paint.' : ''),
            ($isVeneerSimulation && $shadeIsMaxBrightAnchor ? 'Brand-new veneer finish target: the finished veneers must look clean and perfect like newly seated cosmetic porcelain. The main body shade should be luminous neutral white with no cream, no yellow, and no natural-tooth discoloration. Keep translucency delicate and limited to the incisal/bottom edge so the teeth stay dimensional without looking stained.' : ''),
            ...
            ($isVeneerSimulation ? 'Before-vs-after mandate: compared with the original photo, the after must have a clearly improved smile silhouette, cleaner tooth proportions, smoother incisal architecture, more even spacing, and a clean porcelain shade matching the selected target. If the tooth shape or shade looks almost the same as the before, the edit has failed.' : ''),
            ($isVeneerSimulation ? 'Similarity failure rule: if a patient looking at the Compare, B/A, or Zoom view would say the teeth barely changed, the image is unacceptable. Increase the LVI shape change and ensure the porcelain shade clearly matches the selected target until the dental improvement is unmistakable at normal viewing distance.' : ''),
            ($isVeneerSimulation ? 'Keep the veneers uniformly matched to the selected shade\'s value while preserving natural-looking incisal translucency, subtle depth, and polished glaze. The result should look like high-end porcelain, not natural teeth with whitening.' : ''),
            ($isVeneerSimulation ? 'Visible veneer surfaces must be pristine and flawless: zero brown undertone, zero white spots, zero stain halos, zero craze lines, zero cracks, zero mottling, and zero enamel blemishes. If any visible veneer area reads stained or imperfect, the edit is wrong.' : ''),
            ...
            ($isVeneerSimulation && $shadeIsMaxBrightAnchor ? 'Bias veneer previews toward premium cosmetic brightness rather than conservative blending. Natural and Enhanced styles should still read as clearly white veneers on screen, just with different shape language than Hollywood.' : ''),
            ($isVeneerSimulation && $shadeIsMaxBrightAnchor ? 'The whitening jump must be immediately visible at normal screen viewing distance and in the Zoom view. If the after could be mistaken for only a mild cleanup or small whitening pass, it is too subtle - make the value increase stronger while keeping the same patient and realistic porcelain depth.' : ''),
            ($isVeneerSimulation && $shadeIsMaxBrightAnchor ? 'If the model must choose between slightly too natural and slightly too white, choose the whiter result as long as it still looks like dimensional porcelain rather than flat paint.' : ''),
...
final class OpenAISmileDesignImageProvider implements SmileDesignImageProvider
{
    public function createPreview(array $case, array $photos, array $options = []): array
    {
        ...
        $shadeDetail = function_exists('smile_design_shade_detail')
            ? smile_design_shade_detail((string)($options['shade_goal'] ?? $case['shade_goal'] ?? ''), $normalizedStyleKey)
            : ['label' => 'Chromascop 210', 'title' => 'Bright White', 'description' => 'Premium bright white.', 'prompt' => 'Chromascop 210 bright white porcelain shade.'];
        // Was previously referenced below (in the shade prompt block) without ever
        // being computed in this class, so it always evaluated to an undefined-variable
        // warning + null. The Gemini provider already computes this correctly.
        $shadeScreenContract = trim((string)($shadeDetail['screen_contract'] ?? ''));
        // Only Elite Smiles 100 / Chromascop 110 are meant to chase the maximum
        // wow-factor brightness look; see the matching comment in the Gemini provider.
        $shadeIsMaxBrightAnchor = (int)($shadeDetail['value_step'] ?? 4) <= 0;
        $styleAnatomy = function_exists('smile_design_style_generation_guidance') ? smile_design_style_generation_guidance($normalizedStyleKey) : '';
        $customRequest = trim((string)($options['custom_request'] ?? ''));
        $precisionMode = trim((string)($options['precision_mode'] ?? ($options['precision_controls']['precision_mode'] ?? 'balanced')));
        ...
        $promptParts = [
            ...
            ($isVeneerSimulation ? 'Default veneer material: IPS e.max-style lithium disilicate glass-ceramic. The final teeth should show a glazed ceramic surface with realistic specular highlights, smooth polished body, enamel-like optical depth, and delicate incisal translucency, not flat digital paint or generic whitening.' : ''),
            ($isVeneerSimulation ? 'IPS e.max material rule: use lithium disilicate porcelain veneer finish with clean luminous body shade, glossy ceramic highlights, enamel-like optical depth, smooth polished finish, and subtle incisal translucency only at the very bottom edge. Absolutely zero cream, beige, gray, stains, mottling, patches, old enamel shadows, or natural-tooth color may remain on the visible veneer faces.' : ''),
            ($isVeneerSimulation ? 'Material failure rule: if the veneers look flat, opaque, chalky, or like solid white paint instead of genuine glass-ceramic porcelain with real light transmission and depth, the material rendering has failed. The surface must show natural light scattering and translucent depth at the incisal edge — like real e.max lithium disilicate glass-ceramic, not a flat matte or plastic finish — regardless of which shade is selected.' : ''),
            ($isVeneerSimulation ? 'Clinical crown length target: give each upper central incisor a full, age-appropriate clinical crown length of roughly 10.5 mm to 11 mm, in a 4:5 width-to-length ratio where the vertical height clearly dominates the horizontal width. When the patient\'s own teeth are worn short, lengthen them toward that target instead of preserving the short display - restoring worn length is part of the treatment being previewed, not an exaggeration of it.' : ''),
            ($isVeneerSimulation ? 'Length failure rule: crown height must clearly dominate crown width - slender and refined, never bulky. Uniform, square, equal-length "chiclet" blocks across the front teeth are a failure, and so are teeth left so short they read stubby. In this smiling photo the central incisal edges should rest just along the inner curve of the lower lip: never hidden behind the lower lip, and never crossing over it. An exaggerated, denture-like smile line remains a failure in the other direction.' : ''),
            ...
            ($isVeneerSimulation ? 'Make the veneers look like newly delivered premium porcelain: perfectly clean value, consistent body shade, smooth finish, and no natural-tooth defects showing through anywhere on the visible veneer surfaces.' : ''),
            ($isVeneerSimulation && $shadeIsMaxBrightAnchor ? 'Bias veneer previews toward premium cosmetic brightness rather than conservative blending. Natural and Enhanced styles should still read as clearly white veneers on screen, just with different shape language than Hollywood.' : ''),
            ($isVeneerSimulation && $shadeIsMaxBrightAnchor ? 'The brightness increase must be obvious on screen and in the Zoom view. If the after only looks a little cleaner than the before, it is not enough. Make the veneers visibly whiter and more luminous while still dimensional and natural-looking.' : ''),
            ($isVeneerSimulation && $shadeIsMaxBrightAnchor ? 'If the model must choose between slightly too natural and slightly too white, choose the whiter result as long as it still looks like dimensional porcelain rather than flat paint.' : ''),
            ($widerSmileFrontGuidance !== '' ? $widerSmileFrontGuidance : ''),
            ($veneerAngleGuidance !== '' ? $veneerAngleGuidance : ''),
            ($procedureGuidance !== '' ? $procedureGuidance : ''),
