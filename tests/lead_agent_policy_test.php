...
$englishMessageLanguage = lead_language_detect_message_signal('Hello, I would like a consultation for veneers.');
expect_true(($englishMessageLanguage['language'] ?? '') === 'en' && ($englishMessageLanguage['source'] ?? '') === 'inbound_detected', 'A clearly English inbound message should establish a source-backed English preference.');
$ambiguousLanguage = lead_language_detect_message_signal('Yes');
expect_true(($ambiguousLanguage['language'] ?? '') === 'unknown', 'A short ambiguous reply must not relabel the conversation.');
...
expect_true(lead_language_source_priority('landing_page_default') < lead_language_source_priority('inbound_detected'), 'A landing-page default must not override language learned from a conversation.');
expect_true(lead_language_source_priority('landing_page_ui') < lead_language_source_priority('inbound_detected'), 'Legacy unqualified landing-page values must remain weaker than conversation evidence.');
expect_true(lead_language_source_priority('landing_page_selected') > lead_language_source_priority('inbound_explicit'), 'A current explicit language-toggle choice must be allowed to update an older preference.');
...
$landingToggleSource = (string)file_get_contents(dirname(__DIR__) . '/app/landing_pages/partials/bilingual-toggle.php');
$landingHandlerSource = (string)file_get_contents(dirname(__DIR__) . '/app/landing_pages/engine/lead-handler.php');
$landingHeroSource = (string)file_get_contents(dirname(__DIR__) . '/app/landing_pages/partials/sections/hero.php');
$landingQuizSource = (string)file_get_contents(dirname(__DIR__) . '/app/landing_pages/partials/quiz-modal.php');
expect_true(str_contains($leadServiceSource, '$languageRefresh = lead_language_apply_signal($leadId'), 'Duplicate intake refresh must use central source-priority language handling.');
expect_true(str_contains($landingToggleSource, "'landing_page_selected'") && str_contains($landingToggleSource, '[name="preferred_language_source"]'), 'The bilingual toggle must distinguish a patient selection from an automatic default.');
expect_true(str_contains($landingHandlerSource, 'lp_language_preference_source') && !str_contains($landingHandlerSource, "'preferred_language_source' => 'landing_page_ui'"), 'Landing-page POST handling must preserve the qualified language source.');
expect_true(str_contains($landingHandlerSource, 'Landing page display language:') && str_contains($landingHandlerSource, "source === 'landing_page_selected'"), 'An automatic page language must not be recorded in notes as a patient-selected preference.');
expect_true(str_contains($landingHeroSource, 'name="preferred_language_source" value="landing_page_default"'), 'The hero form must label its hidden English value as a weak default.');
expect_true(str_contains($landingQuizSource, 'name="preferred_language_source" value="landing_page_default"'), 'The quiz form must label its hidden English value as a weak default.');
